...
public class ReflectionService 
{
    ...
    public static T CreateWithDefaultConstructor<T>(Type type) where T : class
    {
        T result = null;
        if (type == null)
            throw new ArgumentNullException($"{typeof(ReflectionService).GetTypeName()}.CreateWithDefaultConstructor() => Не удалось создать объект, так как аргумент type содержит ссылку на null.");
