...
public class CanActivateComponent
{
    ...
    public async Task Invoke(HttpContext http, APIAuthorization authorization, NotificationsService user, AuthorizationOptions _options)
    {

        string path = http.Request.Path.ToString();
        if( path=="/"||path==""||path==null) http.Response.Redirect("/Account/Login");
        ...
        try
        {
            ...
            if (path.StartsWith(_options.ActivationRequirePath))
            {
                if (authorization.IsSignin() == false)
                {
                    ...
                    user.ErrorMessage("Доступ запрещён");
                ...
                else
                {
                    ...
                    {
                        user.WarnMessage("Активация учетной записи уже выполнена, повторять эту процедуру не следует");
                ...
                foreach (string rolePath in pair.Value)
                {
                    if (path.StartsWith(rolePath))
                    {
                        ...
                        else
                        {
                            ...
                            if (authorization.InBusinessResource(roleName) == false)
                            {
                                _logger.LogInformation($"denied request to: {path}");
                                user.ErrorMessage("Доступ запрещён");
                                http.Response.Redirect(_options.AccessDeniedPath);
                                return;
                            }
                        }
        ...
        try
        {
            _logger.LogInformation(_next.GetType().Name);
            await _next.Invoke(http);
        }
        catch(Exception ex)
        {
